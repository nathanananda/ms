...
@section('content-user')
    <div class="flex flex-col justify-start my-5">
        <h3 class="font-GabaritoMedium text-2xl">Pengangkatan</h3>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href=""
                            class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2">Pengangkatan</a>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="w-full min-h-screen bg-white rounded-xl p-5">
        ...
        <form action="{{ route('kepsek.laporan.pengangkatan.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex justify-between items-start my-5 space-x-3">
                <div class="w-1/2">
                    ...
                    <div class="mb-4" id="akhir_kontrak_wrapper">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="status_karyawan" name="id_status_karyawan"
                            ...
                            @foreach ($dataStatus as $s)
                                <option value="{{ $s->id_status_karyawan }}">{{ $s->status_karyawan }}</option>
                            @endforeach
                        </select>
                ...
                <div class="w-1/2">
                    <label for="upload_kontrak" class="block text-sm font-medium text-gray-700 mb-2">
                        Upload Dokumen Pengangkatan Kontrak
                    </label>
...
@section('content-script')
        ...
        document.getElementById('nik').addEventListener('change', function() {
            ...
            fetch(apiUrl)
                ...
                .then(data => {
                    ...
                    // Set value select jabatan
                    const jabatanSelect = document.getElementById('jabatan');
                    if (jabatanSelect) jabatanSelect.value = data.id_jabatan || '';

                    // Set value select status karyawan
                    const statusSelect = document.getElementById('status_karyawan');
                    if (statusSelect) statusSelect.value = data.id_status_karyawan || '';
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const statusSelect = document.getElementById('status_karyawan');
            const akhirKontrakWrapper = document.getElementById('akhir_kontrak_wrapper');

            function toggleAkhirKontrak() {
                const selectedText = statusSelect.options[statusSelect.selectedIndex]?.text;
                if (selectedText === 'Pegawai Tetap') {
                    akhirKontrakWrapper.style.display = 'none';
                    document.getElementById('akhir_kontrak').required = false;
                } else {
                    akhirKontrakWrapper.style.display = 'block';
                    document.getElementById('akhir_kontrak').required = true;
                }
            }

            statusSelect.addEventListener('change', toggleAkhirKontrak);
            toggleAkhirKontrak(); // Trigger saat halaman pertama kali dibuka
        });
    </script>
@endsection
